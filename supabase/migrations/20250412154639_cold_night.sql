/*
  ...
  1. New View
    - trend_insights: Aggregates metrics with trend indicators
      - Provides time-based comparisons (daily, weekly, monthly)
      - Calculates change percentages and trend directions
      - Supports anomaly detection via z-scores
      - Includes JSON summary for frontend consumption
...
weekly_metrics AS (
  ...
  GROUP BY organization_id, date_trunc('week', created_at)::date
),
...
language_metrics AS (
  ...
  GROUP BY organization_id, date_trunc('month', created_at)::date, language
),
